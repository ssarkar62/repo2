...
public class UserServiceImpl implements UserService {
				...
				if (BCrypt.checkpw(user.getPassword(), hashPwd)) {
					...
					if (user.getUserId() != null && user.getUserId() > 0 && user.getStatus() != null
						...
						if (!user.getUserRole().equals(Roles.ROLE_SUPER_ADMIN) && user.getStatus().equals((byte) 2)) {
							...
							Integer count = userAuthenticationDAO.saveToken(token, user.getUserId());
							if (count != null && count > 0) {
								user.setToken(token);
								user.setPassword(null);
								response.setStatus(StatusUtil.STATUS_SUCCESS);
								...
								if (loginLog != null) {
									try {
										...
										logger.error("Exception Occured while Saving the LoginLog Details "
												+ e.getMessage());
									}
								}
	...
	public String validateOtp(String jsonOtp) {
		...
		try {
			...
			// call DAO
			Map<String, Object> resultMap = otpLogDAO.validateOtp(otpBean.getUserId());
	...
	public String updateLogoutStatus(Long userId) {
		...
		try {
			if (userId != null) {
				...
				/*
				 * logout status ->0 means not logged out ->1 means logged out
				 * successfuly
				 */
				if (count > 0) {
					response.setStatus(StatusUtil.STATUS_SUCCESS);

...
<div class="container bg-primary text-center jumbotron ">
	...
	<h2 style="font-weight: bold; font-family: cursive;">Search Bus
		For Ticket</h2>
	<form class="form-inline">
		<div class="form-group">
			<label>From</label> <input id="source" type="text" ng-model="from"
				class="form-control" placeholder="Source">
		</div>
		<div class="form-group">
			<label>To</label> <input id="destination" type="text" ng-model="to"
				class="form-control" placeholder="Desitination">
		</div>
		<div class="form-group">
			<label>Journey Date</label> <input id="journeyDate"
				ng-model="journeyDate " placeholder="Journey Date"
				class="form-control">
		</div>
		<div class="form-group">
						...
						<table class="seatTable col-md-12">
							...
							<tr ng-repeat="row in seats[bus.routeId][0].upperDecRow">
								<td ng-repeat="r in row track by $index"><span
									...
									<img  id="{{this.$id}}"  ng-if="r.seatNo!='' && r.isBooked" ng-src={{r.path}}
